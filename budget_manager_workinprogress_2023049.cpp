...
class Priority
{
    ...
    void SetValues_Custom(string name, int div, int subdiv, int prrank, int totalbudget, int prrank_allocation)
    {
        ...
        prioritybudget = totalbudget * (prrank_allocation / 100);
        divisionbudget = Calcdivbudget(prioritybudget, division);
        subdivisionbudget = Calcsubdivbudget(divisionbudget, subdivision);
        PriorityNames[priorityrank-1] = priorityname;
        PriorityRank[priorityrank] = priorityrank;
...
int main()
{
    ...
    if (chosen_budget_option == 1)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            cout << "(e.g 3 subdivisions for daily food expenses, or, 2 sudivisions for toll taxes during daily commute)" << endl;
            do
            {
        ...
        if (chosen_budget_option == 2)
        {
            ...
            cout << "Note that percentages should add up to 100 and each individual percentage must be between 0 and 100." << endl;

            int priority_rank_budgetallocation[5] = {0}; // Initialize array for allocations
            ...
            for (int i = 0; i < 5; i++) 
            {
                ...
                do 
                {
                    cout << "Please enter the percentage for priority rank " << i + 1 << ": ";
                    cin >> input;
                    ...
                    {
                        priority_rank_budgetallocation[i] = input; // Valid input
                        sum_check += input; // Update the sum
                        cout << priority_rank_budgetallocation[i];
                        break;
                    }
                } while (true);
            }
            ...
            cout << name << endl;
            cout << chosen_division << endl;
            cout << chosen_subdivision << endl;
            cout << total_allocated_budget << endl;
            cout << chosen_budget_option << endl;
            cout << priority_rank_budgetallocation[i] << endl;
            cout << i << endl;
    ...
    do 
    {
        cout << "Now you can use one of the following features: " << endl;
        cout << "1. Display all budget." << endl;
        cout << "2. Search for a specific priority by it's name." << endl;
        cout << "3. Save this budget in a txt file on desktop." << endl;
        cout << "4. Delete a previous budget file." << endl;
        cout << "5. Displya history of last accessed Priorities. " << endl;
